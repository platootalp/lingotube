<template>
  <b-card
    ...
    <b-card-header :header-bg-variant="headerBgVariant" :class="headerClasses" v-if="$slots.header">
      <slot name="header">
      </slot>
    </b-card-header>
...
<script>
  export default {
    ...
    props: {
      ...
      headerBgVariant: {
        type: String,
        default: "white", // 默认值为 "dark"，如果不传递则默认为 dark
        description: "Card header background variant (white, light, dark etc)"
      }
    }
  };
</script>
