  ...
  <div>
    ...
    <b-container fluid class="p-4 bg-white">
      <card class="p-4 bg-dark mb-4" header-bg-variant="dark">
        <b-card-title>
          <h1 class="mb-3 text-white">{{ this.levelInfo.name }}</h1>
          <span class="text-secondary">{{ this.levelInfo.description }}</span>
        </b-card-title>
        <b-card-body>
...
<script>
import {getLevelInfo, getPageVideosByLevelId} from "@/api/video";
...
export default {
  ...
  created() {
    this.currentPage = this.$route.query.page || 1; // 或者设置默认值为 1
    this.fetchLevelInfo();
    this.fetchPageVideosByLevelId();
  ...
  methods: {
    ...
    async fetchLevelInfo() {
      try {
        const response = await getLevelInfo(this.id);
        // 检查返回的状态码
        if (response.data.code === 200) {
          this.levelInfo = response.data.data;
        } else {
    ...
    async fetchPageVideosByLevelId() {
      try {
        const response = await getPageVideosByLevelId(this.id, this.currentPage, this.pageSize, this.sort);
        // 检查返回的状态码
        if (response.data.code === 200) {
